...
public class TelegramBillService {
    ...
    public void onBillCreated(BillCreatedEvent event) {
        ...
        final var billId = payload.getFirst();
        final var processingMessage = new SelfMadeTelegramMessage(payload.getSecond());
        ...
        final var billMessageId = getActualMessageId(processingMessage, message);
    ...
    public void onNotRecognizedBill(NotPaymentRelatedEvent event) {
        fallbackEditMessage(NOT_RECOGNIZED_MESSAGE, event);
    }

    @Async
    @EventListener
    public void onProcessingError(ProcessingErrorEvent event) {
        fallbackEditMessage(PROCESSING_ERROR_MESSAGE, event);
    }

    private int getActualMessageId(MaybeInaccessibleMessage processingMessage, BaseResponse billMessageResponse) {
        if (processingMessage.messageId() != Bill.DEFAULT_MESSAGE_ID) {
            return processingMessage.messageId();
        }
        try {
            return ((SendResponse) billMessageResponse).message().messageId();
        } catch (Exception e) {
            log.error("Error while getting message id", e);
            return Bill.DEFAULT_MESSAGE_ID;
        }
    }

    private void fallbackEditMessage(String processingResult, AbstractEvent<Pair<Long, Integer>> event) {
        final var payload = event.getPayload();
        final var chatId = payload.getFirst();
        final var selfMadeTelegramMessage = new SelfMadeTelegramMessage(payload.getSecond());
        final var message = tryUpdateMessage(chatId, selfMadeTelegramMessage, processingResult);
        bot.execute(message);
    }
    ...
    public static class SelfMadeTelegramMessage extends Message {
        ...
        public SelfMadeTelegramMessage(int messageId) {
            super();
